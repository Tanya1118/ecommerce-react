...
const Product = ({product}) => {
  ...
  return (
    <div>
      <div className='border border-[#e4e4e4] h-[300px] mb-4 relative overflow-hidden group transition'>
        <div className='w-full h-full flex justify-center'>
          ...
          <div className='w-[200px] mx-auto flex justify-center items-center'>
            ...
            <img className='max-h-[160px] group-hover:scale-110 traansition duration-300' src={product.images} alt='' />
          </div>
        </div>
        {/*buttons*/}
      ...
      <div>
      <div className='text-sm capitalize text-gray-500'>{product.id}</div>
      <Link to={`/products/${product.id}`}>
      <h2 className='font-semibold mb-1'>{product.title}</h2>
      </Link>
        
        <div className='fontsemibold'>$ {product.price}</div>
